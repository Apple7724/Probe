...
# функция - фильтр по жанрам
def filter_by_genre(data, genre):
    result = [] # пустая таблица result создается
    for i in data: # пробегаем по старой таблице(БД)
        genres = i[4] # создаем переменную и присваиваем ей значение из i- той строки 4 элемент(строка жанров)
        if genre in genres: # если аргумент функции в этой строке....
            result.append(i) #присоединить к новой таблице result i-тую строку
    return result #возвращаем новую таблицу
...
filtered_data = filter_by_genre(oscar_data, 'мелодрама')# вызов фильтра с параметром мелодрама
